...
python play_mujoco_teleop.py --task=T1 --checkpoint=-1 --fps=30 --headless_record
"""
    ...
    if args.render_threads == 1:
        ...
        with tqdm(total=len(batches), desc="Rendering batches") as pbar:
            for batch_idx, batch in enumerate(batches):
                ...
                result = render_batch(
                    batch, batch_idx, None, cfg["asset"]["mujoco_serial_file"], 
        ...
        with tqdm(total=len(batches), desc="Rendering batches") as pbar:
            for i in range(min(args.render_threads, len(batches))):
                p = mp.Process(
                    target=render_batch,
                    args=(batches[next_batch], next_batch, result_queue, cfg["asset"]["mujoco_serial_file"], 
                          camera_settings, recording_state.width, recording_state.height)
                )
...
def initialize_simulation(cfg: Dict[str, Any], args: argparse.Namespace) -> Tuple[SimulationState, RecordingState, RenderingConfig, Optional[Any]]:
    ...
    mj_model = mujoco.MjModel.from_xml_path(cfg["asset"]["mujoco_serial_file"])
    ...
    rendering_config = RenderingConfig(
        ...
        mujoco_file=cfg["asset"]["mujoco_serial_file"]
    )
