...
class T1_7DoF(BaseTask):
    ...
    def _create_envs(self):
        ...
        # Define hand joint patterns to exclude from control
        hand_joint_patterns = [
            "index", "middle", "ring", "pinky", "thumb",  # finger names
            "MCP", "PIP", "DIP",  # joint types
            "_hand_", "_finger_", "_fing_"  # additional hand patterns
        ]
        
        # Identify controlled DOFs (exclude hand joints)
        self.controlled_dof_indices = []
        self.controlled_dof_names = []
        
        for i in range(self.num_dofs):
            is_hand_joint = any(pattern in self.dof_names[i] for pattern in hand_joint_patterns)
            if not is_hand_joint:
                # Check if this joint has a control configuration
                found_control = False
                ...
                if found_control:
                    ...
                    self.controlled_dof_names.append(self.dof_names[i])
        
        self.num_controlled_dofs = len(self.controlled_dof_indices)
        print(f"Found {self.num_dofs} total DOFs, controlling {self.num_controlled_dofs} DOFs")
        print(f"Controlled DOFs: {self.controlled_dof_names}")
        ...
        # Initialize control gains for all DOFs (controlled and uncontrolled)
        ...
        # Set control gains only for controlled DOFs
        for i in range(self.num_dofs):
            is_controlled = i in self.controlled_dof_indices
            if is_controlled:
                found = False
                for name in self.cfg["control"]["stiffness"].keys():
                    if name in self.dof_names[i]:
                        ...
                        break
                if not found:
                    raise ValueError(f"PD gain of controlled joint {self.dof_names[i]} were not defined")
            else:
                # Set minimal control for uncontrolled DOFs (hands)
                self.dof_stiffness[:, i] = 0.0
                self.dof_damping[:, i] = 0.1  # Small damping to prevent instability
    ...
    def _init_buffers(self):
        ...
        # Verify that the configured num_actions matches our controlled DOFs
        self.num_actions = self.num_controlled_dofs
        ...
        # Set default positions for all DOFs (controlled and uncontrolled)
        for i in range(self.num_dofs):
            found = False
    ...
    def step(self, actions):
        ...
        # Create full DOF targets tensor (all DOFs)
        dof_targets = self.default_dof_pos.repeat(self.num_envs, 1)
        
        # Map actions to controlled DOFs only
        for i, dof_idx in enumerate(self.controlled_dof_indices):
            dof_targets[:, dof_idx] = self.default_dof_pos[:, dof_idx] + self.cfg["control"]["action_scale"] * self.actions[:, i]
    ...
    def _compute_observations(self):
        ...
        # Extract controlled DOF positions and velocities
        controlled_dof_pos = self.dof_pos[:, self.controlled_dof_indices]
        controlled_dof_vel = self.dof_vel[:, self.controlled_dof_indices]
        controlled_default_pos = self.default_dof_pos[:, self.controlled_dof_indices]
        ...
        self.obs_buf = torch.cat(
            (
                ...
                apply_randomization(controlled_dof_pos - controlled_default_pos, self.cfg["noise"].get("dof_pos")) * self.cfg["normalization"]["dof_pos"],
    ...
    def _reward_torques(self):
        # Penalize torques only for controlled DOFs
        controlled_torques = self.torques[:, self.controlled_dof_indices]
        return torch.sum(torch.square(controlled_torques), dim=-1)

    def _reward_dof_vel(self):
        # Penalize dof velocities only for controlled DOFs
        controlled_dof_vel = self.dof_vel[:, self.controlled_dof_indices]
        return torch.sum(torch.square(controlled_dof_vel), dim=-1)

    def _reward_dof_acc(self):
        # Penalize dof accelerations only for controlled DOFs
        controlled_dof_vel = self.dof_vel[:, self.controlled_dof_indices]
        controlled_last_dof_vel = self.last_dof_vel[:, self.controlled_dof_indices]
        return torch.sum(torch.square((controlled_last_dof_vel - controlled_dof_vel) / self.dt), dim=-1)
    ...
    def _reward_dof_pos_limits(self):
        # Penalize dof positions too close to the limit only for controlled DOFs
        controlled_dof_pos = self.dof_pos[:, self.controlled_dof_indices]
        controlled_dof_pos_limits = self.dof_pos_limits[self.controlled_dof_indices]
        
        lower = controlled_dof_pos_limits[:, 0] + 0.5 * (1 - self.cfg["rewards"]["soft_dof_pos_limit"]) * (
            controlled_dof_pos_limits[:, 1] - controlled_dof_pos_limits[:, 0]
        )
        upper = controlled_dof_pos_limits[:, 1] - 0.5 * (1 - self.cfg["rewards"]["soft_dof_pos_limit"]) * (
            controlled_dof_pos_limits[:, 1] - controlled_dof_pos_limits[:, 0]
        )
        return torch.sum(((controlled_dof_pos < lower) | (controlled_dof_pos > upper)).float(), dim=-1)

    def _reward_dof_vel_limits(self):
        # Penalize dof velocities too close to the limit only for controlled DOFs
        controlled_dof_vel = self.dof_vel[:, self.controlled_dof_indices]
        controlled_dof_vel_limits = self.dof_vel_limits[self.controlled_dof_indices]
        
        # clip to max error = 1 rad/s per joint to avoid huge penalties
        return torch.sum(
            (torch.abs(controlled_dof_vel) - controlled_dof_vel_limits * self.cfg["rewards"]["soft_dof_vel_limit"]).clip(min=0.0, max=1.0),
            dim=-1,
        )

    def _reward_torque_limits(self):
        # Penalize torques too close to the limit only for controlled DOFs
        controlled_torques = self.torques[:, self.controlled_dof_indices]
        controlled_torque_limits = self.torque_limits[self.controlled_dof_indices]
        
        return torch.sum(
            (torch.abs(controlled_torques) - controlled_torque_limits * self.cfg["rewards"]["soft_torque_limit"]).clip(min=0.0),
            dim=-1,
        )

    def _reward_torque_tiredness(self):
        # Penalize torque tiredness only for controlled DOFs
        controlled_torques = self.torques[:, self.controlled_dof_indices]
        controlled_torque_limits = self.torque_limits[self.controlled_dof_indices]
        
        return torch.sum(torch.square(controlled_torques / controlled_torque_limits).clip(max=1.0), dim=-1)

    def _reward_power(self):
        # Penalize power only for controlled DOFs
        controlled_torques = self.torques[:, self.controlled_dof_indices]
        controlled_dof_vel = self.dof_vel[:, self.controlled_dof_indices]
        
        return torch.sum((controlled_torques * controlled_dof_vel).clip(min=0.0), dim=-1)
