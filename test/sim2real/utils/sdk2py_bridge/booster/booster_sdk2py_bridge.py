...
class BoosterSdk2Bridge(BasicSdk2Bridge):
    ...
    def _init_sdk_components(self):
        ...
        if robot_type == "t1_23dof" or robot_type == "t1_29dof":
            ...
            raise ValueError(f"Invalid robot type '{robot_type}'. Expected 't1_23dof' or 't1_29dof'.")
        ...
        self.low_state_subscriber = B1LowStateSubscriber(self.LowCmdHandler)
        self.low_cmd_publisher.InitChannel()
        self.low_state_subscriber.InitChannel()
        # TODO: wireless controller for booster

    def LowCmdHandler(self, msg):
        """Handle Booster low-level state messages."""
        if msg:
            self.low_state = msg
    ...
    def PublishLowState(self):
        ...
        if self.use_sensor:
            ...
            for i in range(num_motors):
                ...
                m.dq = qvel[6 + i]
                m.ddq = qacc[6 + i]
                m.tau_est = actuator_force[6 + i] if self.free_base else actuator_force[i]
        ...
        imu.gyro = gyro
        imu.acc = acc
        # Note: State data is updated in self.low_state, no need to publish it